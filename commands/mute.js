...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!client.config.muteMembers.some(id => message.member.roles.cache.has(id))&& (!message.member.hasPermission("ADMINISTRATOR"))) {
            return message.channel.send(embed.setDescription('Komutu kullanan kullanıcıda yetki bulunmamakta!').setFooter(`Solve?`)).then(x => x.delete({ timeout: 5000 }))
        }
        ...
        moment.locale('tr')
        if (!member) return message.channel.send(embed.setDescription("Kullanıcı etiketlenmedi veya bulunamadı!").setFooter(`Solve?`)).then(x => x.delete({ timeout: 5000 }))
        if (member.roles.highest.position >= message.member.roles.highest.position) {
            return message.channel.send(embed.setDescription("Belirttiğin kullanıcı seninle aynı yetkide veya senden üstün!").setFooter(`Solve?`)).then(x => x.delete({ timeout: 5000 }))
        }
        let reason = args.slice(2).join(' ');
        if (!reason || reason.split('.').join('').length < 5) return message.channel.send(embed.setDescription("Lütfen Düzgün Bir Şekilde Sebe Belirtin.").setFooter(`Solve?`)).then(x => x.delete({ timeout: 5000 }))
        let time = args[1]
        let mutezaman = ms(`${time}`)
        const filter = (reaction, user) => {
            return ['📄', '🔇'].includes(reaction.emoji.name) && user.id == message.author.id;
        };
        await message.channel.send(embed.setDescription(`${member}, adlı kullanıcıyı yazı kanallarında susturmak için '📄' emojisine seste sustrmak için '🔇'emojisine basınız!`).setFooter(`Solve?`)).then(async msg => {
            await msg.react("📄")
            await msg.react("🔇")
            msg.awaitReactions(filter, {
                ...
                if (reaction.emoji.name == "📄") {
                    ...
                    return msg.edit(embed.setDescription(`${member} adlı kullanıcı **yazı** kanallarında **${reason}** sebebiyle **${time}** süresince susturuldu`).setFooter(`Solve?`)).then(() => {
                        channel.send(embed2.setTitle('Kullanıcı Susturuldu').setDescription(`Yetkili: ${message.author} - \`${message.author.id}\` \n Susturulan: ${member} - \`${member.id}\` \n Sebep: ${reason} \n Süre: ${time} \n Tip: Text Kanalları \n Tarih: **${moment().format('LLL')}**`).setFooter(`Solve?`))
                        if (time) setTimeout(() => {
                    ...
                    member.voice.kick();
                    return msg.edit(embed.setDescription(`${member} adlı kullanıcı **ses** kanallarında **${reason}** sebebiyle **${time}** süresince susturuldu`).setFooter(`Solve?`)).then(() => {
                        channel.send(embed2.setTitle('Kullanıcı Susturuldu').setDescription(`Yetkili: ${message.author} - \`${message.author.id}\` \n Susturulan: ${member} - \`${member.id}\` \n Sebep: ${reason} \n Süre: ${time} \n Tip: Ses Kanalları \n Tarih: **${moment().format('LLL')}**`).setFooter(`Solve?`))
                        if (time) setTimeout(() => {
                            member.roles.remove(client.config.voicemuteRoles)
                            channel.send(embed2.setTitle('Kullanıcı Susturması Açıldı').setDescription(`${member} Adlı Kullanıcının ses mutesi açıldı`)).setFooter(`Solve?`)
                        }, mutezaman)
                    })
                }
            })
