...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        let name = args[1]
        if (!name) return message.channel.send(embed.setDescription("Kullanıcı için bi isim yazılmak zorunda!").setFooter(`Solve?`)).then(x => x.delete({ timeout: 5000 }))

        let age = args[2]
        if (!age) return message.channel.send(embed.setDescription("Kullanıcı için bir yaş yazılmak zorunda!").setFooter(`Solve?`)).then(x => x.delete({ timeout: 5000 }))
