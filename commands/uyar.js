...
module.exports = {
    ...
    run: async(client, message, args) => {
        if (!client.config.jailMembers.some(id => message.member.roles.cache.has(id))&& (!message.member.hasPermission("ADMINISTRATOR"))) {
            return message.channel.send(embed.setDescription("Komutu kullanan kullanıcıda gerekli yetki bulunmuyor!").setFooter(`Solve?`))
        }
        ...
        moment.locale('tr')
        if (!member) return message.channel.send(embed.setDescription("Kullanıcı bulunmaadı eya etiketlenmedi!").setFooter(`Solve?`))
        let reason = args.slice(1).join(' ')
        if (!reason) return message.channel.send(embed.setDescription("kullanıcıyı neden banlamak istiyorsunuz.").setFooter(`Solve?`))
