...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!member || isNaN(member) || !banList.has(member)) {
            return message.channel.send(embed.setDescription(` Kullanıcı id hatalı veya kullanıcı yasaklı değil!`).setFooter(`Solve?`))
        }
        ...
        channel.send(embed.setDescription(`${message.author} adlı yetkili tarafından <@${member}> adlı kullanıcının banını kaldırıldı!`).setFooter(`Solve?`))
    }
}
