...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!client.config.jailMembers.some(id => message.member.roles.cache.has(id))) {
            return message.channel.send(embed2.setDescription("Bu Komut İçin Yetkin Bulunmuyor.").setFooter(`Solve?`))
        }
        let member = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
        if (!member) return message.channel.send("Bir Kullanıcı belirt!");
        if (!db.get(`${member.id}_sicil`)) return message.channel.send("Bu kullanıcının hiçbir cezası/uyarısı yok.");
